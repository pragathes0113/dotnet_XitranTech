...
public partial class PrintProductPaymentInvoice : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (HttpContext.Current.Session["PaymentID"] != null)
                hdnBillNo.Value = HttpContext.Current.Session["PaymentID"].ToString();
            else
                hdnBillNo.Value = "-1";
    ...
    private void Databind()
    {
        ...
        dsData = VHMS.DataAccess.VHMSReports.PrintProductPaymentInvoice(Convert.ToInt32(hdnBillNo.Value));
        try
        {
            ReportDataSet dsReportData = new ReportDataSet();
            dsReportData = LoadData();
            rprt = new ReportDocument();
            rprt.Load(Server.MapPath("~/Reports/rptProductPaymentReport.rpt"));
